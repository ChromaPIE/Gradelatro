...
loadout_state.loadout.license = 5
local reset_transports = DebugTools.reset_transports(loadout_state)
H.assert_equal(reset_transports.ok, true, "transport reset succeeds")
H.assert_equal(reset_transports.removed, 5, "transport reset reports removed purchases")
H.assert_equal(loadout_state.loadout.transports.gold, nil, "transport reset removes gold")
H.assert_equal(loadout_state.loadout.active_transport, nil, "transport reset clears active transport")
H.assert_equal(loadout_state.loadout.license, 5, "transport reset keeps license")
...
H.assert_equal(DebugTools.reset_transports(nil).ok, false, "transport reset without collection rejected")
...
local reset_message = DebugTools.dispatch(ns, { "transport", "reset" })
H.assert_true(reset_message:find("Reset transports", 1, true) ~= nil, "transport reset dispatch reports reset")
H.assert_equal(ns.collection.loadout.transports.purple, nil, "transport reset dispatch clears purchases")
H.assert_equal(ns.collection.loadout.active_transport, nil, "transport reset dispatch clears active")
